...
class FONTS():
    ...
    def character(self, asc,xt,yt,size,color):  # Single character size is size: 1 or 2
        ...
        for ii in range(5):
            line = self.FONT[code + ii]
            for yy in range(8):
                if (line >> yy) & 0x1:
                    self.LCD.pixel(ii*size+xt,yy*size+yt,color) 
                    if size > 1:
                        self.LCD.pixel(ii*size+xt+1,yy*size+yt,color)
                        self.LCD.pixel(ii*size+xt,yy*size+yt+1,color)
                        self.LCD.pixel(ii*size+xt+1,yy*size+yt+1,color)
                    if size == 3:
                        self.LCD.pixel(ii*size+xt,  yy*size+yt+2,color)
                        self.LCD.pixel(ii*size+xt+1,yy*size+yt+2,color)
                        self.LCD.pixel(ii*size+xt+2,yy*size+yt+2,color)
                        self.LCD.pixel(ii*size+xt+2,yy*size+yt,color)
                        self.LCD.pixel(ii*size+xt+2,yy*size+yt+1,color)
                                        
    def text(self,asci,xx,yy,size,color):  # Text string
