...
def character(LCD:displayType, asc,xt,yt,size,color):  # Single character size is size: 1 or 2
    code = asc * 5    # 5 bytes per character
    for ii in range(5):
        line = FONT[code + ii]
        for yy in range(8):
            if (line >> yy) & 0x1:
                LCD.pixel(ii*size+xt,yy*size+yt,color) 
                if size > 1:
                    LCD.pixel(ii*size+xt+1,yy*size+yt,color)
                    LCD.pixel(ii*size+xt,yy*size+yt+1,color)
                    LCD.pixel(ii*size+xt+1,yy*size+yt+1,color)
                if size == 3:
                    LCD.pixel(ii*size+xt,  yy*size+yt+2,color)
                    LCD.pixel(ii*size+xt+1,yy*size+yt+2,color)
                    LCD.pixel(ii*size+xt+2,yy*size+yt+2,color)
                    LCD.pixel(ii*size+xt+2,yy*size+yt,color)
                    LCD.pixel(ii*size+xt+2,yy*size+yt+1,color)
                                    
def text(LCD:displayType,asci,xx,yy,size,color):  # Text string
    move = 6*size - int(size/2)
    for letter in(asci):
        asci = ord(letter)
        character(LCD,asci,xx,yy,size,color)
        xx = xx + move

# def cntr_st(LCD:displayType,s,y,size,color): # Centres text on line y
#     width = 6*size - int(size/2)
#     gap = int((LCD.width - len(s) * width)/2)
#     prnt_st(LCD,s,gap,y,size,color)
